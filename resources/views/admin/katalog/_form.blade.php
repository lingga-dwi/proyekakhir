...
<div class="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
    <div class="space-y-6">
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div class="border-b border-slate-100 pb-4">
                <h2 class="font-semibold text-slate-950">Informasi Desain</h2>
                <p class="mt-1 text-sm text-slate-500">Informasi yang akan tampil pada katalog publik.</p>
            </div>
            ...
            <div class="mt-5 grid gap-5 md:grid-cols-2">
                ...
                <label class="block">
                    <span class="text-sm font-medium text-slate-700">Ukuran ruangan (m&sup2;)</span>
                    <input type="number" name="room_size" value="{{ old('room_size', $katalog->room_size ?? '') }}" min="1" max="100000" class="{{ $fieldClass }} @error('room_size') border-red-400 @enderror" placeholder="Opsional">
                    @error('room_size')<span class="mt-1 block text-xs text-red-600">{{ $message }}</span>@enderror
                </label>

                <label class="block md:col-span-2">
                    <span class="text-sm font-medium text-slate-700">Gaya desain</span>
                ...
                <label class="block md:col-span-2">
                    <span class="text-sm font-medium text-slate-700">Cerita inspirasi</span>
                    <textarea name="inspiration_story" rows="4" maxlength="5000" class="{{ $fieldClass }} @error('inspiration_story') border-red-400 @enderror" placeholder="Opsional: ceritakan kebutuhan atau ide di balik desain ini.">{{ old('inspiration_story', $katalog->inspiration_story ?? '') }}</textarea>
        ...
        <section class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div class="border-b border-slate-100 pb-4">
                <h2 class="font-semibold text-slate-950">Media</h2>
            ...
            <div class="mt-5 space-y-6">
                ...
                <label class="block">
                    <span class="text-sm font-medium text-slate-700">{{ $editing && $katalog->gambar_utama ? 'Ganti gambar utama' : 'Gambar utama' }}</span>
                ...
                @if($editing && count($katalog->galeri_gambar ?? []) > 0)
                    <fieldset>
                        <legend class="text-sm font-medium text-slate-700">Galeri saat ini</legend>
                ...
                <label class="block">
                    <span class="text-sm font-medium text-slate-700">{{ $editing ? 'Tambah gambar galeri' : 'Galeri tambahan' }}</span>
    ...
    <aside class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm xl:sticky xl:top-24">
        <h2 class="font-semibold text-slate-950">Publikasi</h2>
        ...
        <div class="mt-5 rounded-xl bg-slate-50 p-4 text-xs leading-5 text-slate-500">
            <p class="font-semibold text-slate-700">Syarat publikasi</p>
            <p class="mt-1">Nama, kategori, deskripsi, dan gambar utama harus sudah tersedia.</p>
        </div>
